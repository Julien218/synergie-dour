...
export function serveStatic(app: Express) {
  ...
  if (!fs.existsSync(distPath)) {
    ...
    console.log(`[serveStatic] Build directory found: ${distPath}`);
    const files = fs.readdirSync(distPath);
    console.log(`[serveStatic] Contents: ${files.join(", ")}`);
  }
  ...
  app.use("*", (_req, res) => {
    ...
    if (!fs.existsSync(indexPath)) {
      ...
      res.status(404).send("index.html not found — ensure the client has been built");
      return;
    }
