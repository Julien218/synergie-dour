...
import { Search, Users, ToggleLeft, ToggleRight, Download } from "lucide-react";

// Données importées depuis Google Sheets (884 commerces Synergie Dour)
const RAW_DATA = [
  {
    "id": 1,
...
export default function ListeClients() {
  const [clients, setClients] = useState(() =>
    RAW_DATA.map((c: any) => ({ ...c, actif: true }))
  );
  ...
  const exportCSV = () => {
    ...
    const rows = [
      ["ID", "Type", "Nom", "Adresse", "Localité", "Téléphone", "Email"],
      ...actifs.map((c: any) => [c.id, c.type, c.nom, c.adresse, c.localite, c.telephone, c.email]),
    ];
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-[#001a3d] text-white">
                ...
                <th className="px-4 py-3 text-left font-semibold">Nom / Commerce</th>
              ...
              {filtered.map((c: any, i: number) => (
                <tr
                  ...
                  className={`border-t border-gray-100 transition-colors ${
                    c.actif ? "hover:bg-green-50/30" : "bg-gray-50 opacity-60 hover:bg-gray-100"
                  }`}
                >
                  ...
                  <td className="px-4 py-3">
                    ...
                    <div className="text-xs text-gray-400 mt-0.5 lg:hidden">{c.localite}</div>
                  </td>
                  ...
                  <td className="px-4 py-3 hidden lg:table-cell text-gray-600 text-xs">
                    <div>{c.adresse}</div>
                    <div className="text-gray-400">{c.localite}</div>
                  </td>
