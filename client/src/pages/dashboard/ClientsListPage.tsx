...
export default function ClientsListPage() {
  const [clients, setClients] = useState<any[]>(
    RAW_CLIENTS.map((c) => ({ ...c, actif: true }))
  );
  ...
  const toggleAll = (val: boolean) => {
    setClients((prev) => prev.map((c) => ({ ...c, actif: val })));
  };

  const activeCount = clients.filter((c) => c.actif).length;
  ...
  const exportCSV = () => {
    ...
    a.href = url;
    a.download = `clients_actifs_${new Date().toISOString().slice(0,10)}.csv`;
    a.click();
    ...
    <div className="min-h-screen bg-[#001a3d] text-white p-4 md:p-6">
      <div className="mb-6">
        ...
        <div className="mt-4 flex flex-col sm:flex-row gap-3">
          ...
          <select
            value={filterActif}
            onChange={(e) => setFilterActif(e.target.value as "all"|"on"|"off")}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-[#002366] border-b border-[#D4AF37]/30">
              <th className="px-3 py-3 text-left text-[#D4AF37] font-semibold w-10">#</th>
              ...
              <th className="px-3 py-3 text-left text-[#D4AF37] font-semibold hidden xl:table-cell">Tél.</th>
              ...
              <tr
                ...
                className={`border-b border-[#D4AF37]/10 transition-colors ${
                  c.actif
                    ...
                    : "bg-[#0a0a1a] opacity-40 hover:bg-[#111130]"
                }`}
              >
